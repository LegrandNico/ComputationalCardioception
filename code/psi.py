...
#     -------
#         y :
#             Conditional probabilities p(response | mu, sigma, gamma, lambda, x).
#     """
#     # Unpack parameters
...
#         p = ones - np.exp(
#             -np.power(
#                 (np.multiply(ones, 10.0)), (np.multiply(sigma, (np.subtract(x, mu))))
#             )
#         )
#     elif psyfun == "Weibull":
#         # F(x; mu, sigma)
...
#     Find the stimulus intensity with minimum expected entropy for each trial, to
#     determine the psychometric function.

#     Use the method described in [1] and psi-marginal staircase as described in [3].
#     Implementation based on the psi-marginal method in the Palamedes toolbox
#     (version 1.8.1) for Matlab [2]. This Python implementation is based on [4].

#     References
#     ----------
#     ..[1] Kontsevich, L. L. & Tyler, C. W. (1999). Bayesian adaptive estimation of
#         psychometric slope and threshold. Vision Research, 39, 2729-2737.
#     ..[2] Prins, N & Kingdom, F. A. A. (2009). Palamedes: Matlab routines for analyzing
#         psychophysical data. http://www.palamedestoolbox.org
#     ..[3] Prins, N. (2013). The psi-marginal adaptive method: How to give nuisance
#         parameters the attention they deserve (no more, no less). Journal of Vision,
#         13(7):3, 1-17.
#     ..[4] https://github.com/NNiehof/Psi-staircase/tree/master

#     Arguments
#     ---------
#     stimRange :
#         range of possible stimulus intensities.
#     Pfunction : type of psychometric function to use. `'cGauss'` for cumulative Gaussian
#         or `'Gumbel'` for Gumbel (log Weibull) psychometric function.
#     nTrials :
#         number of trials
#     threshold :
#         (alpha) range of possible threshold values to search.
#     thresholdPrior :
#         Type of prior probability distribution to use. `('normal',0,1)` for normal
#         distribution, mean and standard deviation. `('uniform', None)` for uniform
#         distribution, mean and standard deviation not defined.

#         .. seealso::

#             slopePrior, guessPrior, lapsePrior.
...
#         See thresholdPrior.
#     marginalize (bool) :
#         If `True`, marginalize out the lapse rate and guessing rate before finding the
#         stimulus intensity of lowest expected entropy. This uses the Prins (2013) method
#         to include the guessing and lapse rate into the probability disctribution. These
#         rates are then marginalized out, and only the threshold and slope are included
#         in selection of the stimulus intensity. If `False`, lapse rate and guess rate
#         are included in the selection of stimulus intensity.

#     Example
#     -------
#     Create a psi object instance with all relevant arguments. Selecting a correct search
#     space for the threshold, slope, guessing rate and lapse rate is important for the
#     psi procedure to function well. If an estimate for one of the parameters ends up at
#     its (upper or lower) limit, the result is not reliable, and the procedure should be
#     repeated with a larger search range for that parameter.

#     Example:
#         >>> s   = range(-5,5) # possible stimulus intensities
#         obj = Psi(s)

#     The stimulus intensity to be used in the current trial can be found in the field
#     xCurrent.
...
#     After each trial, update the psi staircase with the subject response, by calling
#     the addData method.
...
#         self,
#         stimRange,
#         Pfunction: str = "cGauss",
#         nTrials: int = 50,
#         threshold: Optional[tuple] = None,
#         thresholdPrior: tuple = ("uniform", None),
#         slope: Optional[tuple] = None,
...
#         self.marginalize = (
#             marginalize  # marginalize out nuisance parameters gamma and lambda?
#         )
...
#         self.gammaEQlambda = all(
#             (
#                 all(self.guessRate == self.lapseRate),
#                 all(self.priorGamma == self.priorLambda),
#             )
#         )
#         # likelihood: table of conditional probabilities
#         # p(response | alpha, sigma, gamma, lambda, x)
...
#                 pf(
#                     cartesian(
#                         (self.threshold, self.slope, self.lapseRate, self.stimRange)
#                     ),
#                     psyfun=Pfunction,
#                 ),
#                 self.dimensions,
...
#                 np.prod(
#                     cartesian((self.priorMu, self.priorSigma, self.priorLambda)), axis=1
#                 ),
#                 self.dimensions[:-1],
#             )
#         else:
...
#         -------
#         entropy :
#             Shannon entropy of posterior for each stimuli.
...
#             while (
#                 postDims > 3
#             ):  # marginalize out second-to-last dimension, last dim is x
...
#         dimSum = tuple(
#             range(postDims - 1)
#         )  # dimensions to sum over. also a Chinese dish
...
#         Minimum Shannon entropy is used as selection criterion for the stimulus
#         intensity in the upcoming trial.
...
#         self.sumAxes = tuple(
#             range(self.nDims)
#         )  # sum over all axes except the stimulus intensity axis
...
#         # Posterior probability of parameter values
#         # given stimulus intensity x and response r
#         # p(alpha, sigma | x, r)
...
#         self.expectEntropy = np.multiply(
#             self.entropySuccess, self.pSuccessGivenx
#         ) + np.multiply(self.entropyFailure, self.pFailureGivenx)
#         self.minEntropyInd = np.argmin(
#             self.expectEntropy
#         )  # index of smallest expected entropy
#         self.xCurrent = self.stimRange[
#             self.minEntropyInd
#         ]  # stim intensity at minimum expected entropy
...
#         """Add the most recent response.

#         Parameters
#         ----------
#         response :
#             1: correct/right - 0: incorrect/left.
...
#         # Keep the posterior probability distribution
#         # that corresponds to the recorded response
#         if response == 1:
#             # select the posterior that corresponds
#             # to the stimulus intensity of lowest entropy
...
#         self.stdThreshold = np.sqrt(
#             np.sum(
#                 np.multiply((self.threshold - self.eThreshold) ** 2, self.pThreshold)
#             )
#         )
#         self.stdSlope = np.sqrt(
#             np.sum(np.multiply((self.slope - self.eSlope) ** 2, self.pSlope))
#         )
#         self.stdLapse = np.sqrt(
#             np.sum(np.multiply((self.lapseRate - self.eLapse) ** 2, self.pLapse))
#         )
#         self.stdGuess = np.sqrt(
#             np.sum(np.multiply((self.guessRate - self.eGuess) ** 2, self.pGuess))
#         )
...
#     def plot(
#         self,
#         muRef: Optional[float] = None,
#         sigmaRef: Optional[float] = None,
#         lapseRef: Optional[float] = None,
#         guessRef: Optional[float] = None,
#         save: bool = False,
#     ):
#         """Plot marginal distributions.

#         Title of the parameter posteriors indicate the mean +- sd of parameters marginal
#         posterior.
...
#             poststd = np.sqrt(
#                 np.sum(self.likelihood**2 * self.pdfND, axis=(0, 1, 2)) - postmean**2
#             )  # std
#         else:
#             postmean = np.sum(self.likelihood * self.pdfND, axis=(0, 1, 2, 3))  # mean
#             poststd = np.sqrt(
#                 np.sum(self.likelihood**2 * self.pdfND, axis=(0, 1, 2, 3)) - postmean**2
#             )  # std
...
#             "Posterior "
#             + r"$\mu$="
#             + str(np.round(self.eThreshold, 3))
#             + r" $\pm$ "
#             + str(np.round(self.stdThreshold, 3))
#         )
...
#             "Posterior "
#             + r"$\lambda$="
#             + str(np.round(self.eLapse, 3))
#             + r" $\pm$ "
#             + str(np.round(self.stdLapse, 3))
#         )
...
#             "Posterior "
#             + r"$\sigma$="
#             + str(np.round(self.eSlope, 3))
#             + r" $\pm$ "
#             + str(np.round(self.stdSlope, 3))
#         )
